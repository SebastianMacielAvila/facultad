...
<html>
	<head>
		<meta charset="UTF-8">
		<title>Formulario de edición de alumnos</title>
	</head>
	<body>
<?php
...
if($con){
	$query = "select nombre_alumno, apepaterno_alumno, apematerno_alumno, correoe_alumno, tel_alumno from alumnos where id_alumnos='".$id."'";
	$query = pg_query($con,$query);
	$resultado = pg_fetch_assoc($query);
	//print_r($resultado);
?>
		<h1>Formulario de alta de alumnos</h1>
		<p>Por favor ingrese los siguientes datos para registrar los alumnos:</p>
		<form name="editar" action="actualiza_alumno.php" method="post">
			...
			<input type="text" name="apaterno" value="<?php echo $resultado['apepaterno_alumno']; ?>"><br/>
			<label for="amaterno">Apellido materno: </label>
			<input type="text" name="amaterno" value="<?php echo $resultado['apematerno_alumno']; ?>"><br/>
